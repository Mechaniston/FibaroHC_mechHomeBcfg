...
local plugBedID = 15;
local plugAddBRID = 228;
local switchLightAddBRID = 230;

local debug = true;

-- GET ENVS --

local switchDecorLight, switchDecorLightModTime = fibaro:get(lightDecorID, "value");
local switchBRDecorLight, switchBRDecorLightModTime = fibaro:get(lightDecorPPBRID, "value");
local valuePlugAddBR = fibaro:getValue(plugAddBRID, "value");
local valueSwitchLightAddBR = fibaro:getValue(switchLightAddBRID, "value");

-- PROCESS --

if ( tonumber(fibaro:getValue(bedID, "value")) > 0 )
  then -- bed is opened
  ...
  if (tonumber(fibaro:getValue(lightBRbedID, "value")) > 0)
  then
